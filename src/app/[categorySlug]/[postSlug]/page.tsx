...
async function fetchPost(slug: string): Promise<Post | null> {
  try {
    ...
    console.log(`API response for post slug ${slug}:`, JSON.stringify(response.data, null, 2));
...
export default async function PostOrSubCategoryPage({
  ...
  if (possibleSubCategory && possibleSubCategory.parent) {
    const parentCategory = typeof possibleSubCategory.parent === "string"
    ...
    return (
      ...
      >
        ...
        <div className="post-first-tag">
          ...
          <p className="text-gray-600 text-center">No posts available in this subcategory.</p>
        )}
      </div>
    );
  }
  ...
  for (const latestPost of latestPosts) {
    ...
    if (latestCategory?.parent && typeof latestCategory.parent === "string") {
      if (!parentCategoriesMap[latestCategory.parent]) {
        parentCategoriesMap[latestCategory.parent] = await fetchParentCategory(latestCategory.parent);
      }
    }
  }
  ...
  <div className="d-flex gap al-cn social-media-icon fl-wrap-mob">
    {/* WhatsApp Share */}
    <a
      href={`https://api.whatsapp.com/send?text=${encodeURIComponent(post.title)}%20${encodeURIComponent(`https://www.dinasuvadu.com/${categorySlug}/${postSlug}`)}`}
      target="_blank"
    ...
    {/* Facebook Share */}
    <a
      href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://www.dinasuvadu.com/${categorySlug}/${postSlug}`)}`}
      target="_blank"
      rel="noopener noreferrer"
    ...
    {/* Twitter (X) Share */}
    <a
      href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`https://www.dinasuvadu.com/${categorySlug}/${postSlug}`)}`}
      target="_blank"
      rel="noopener noreferrer"
      id="twitter-logo"
    ...
    {/* LinkedIn Share */}
    <a
      href={`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(`https://www.dinasuvadu.com/${categorySlug}/${postSlug}`)}&title=${encodeURIComponent(post.title)}`}
      target="_blank"
      rel="noopener noreferrer"
    ...
    >
      <svg width="110" height="40" viewBox="0 0 260 60" fill="none">
        ...
        ></path>
          ...
          {/* Hero Image */}
          {(post.layout?.[0]?.blockType === "mediaBlock" && post.layout[0].media) || post.heroImage ? (
              ...
              <div className="space-y-4" style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
                {latestPosts.map((latestPost) => {
                  ...
                  if (latestCategory?.parent) {
                    const parent = typeof latestCategory.parent === "string"
    ...
    for (const category of categories) {
      if (category.parent) {
        ...
        if (parent) {
          ...
          console.log(`Generated path for subcategory: ${parent.slug}/${category.slug}`);
        }
      }
    }
